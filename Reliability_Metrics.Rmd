...
Jira <- read_csv("EMBR_Jira_Data.csv")
```
# Tidy Data
```{r resolved_date}
...
```
```{r IQR, echo=FALSE}
Q1ts <- quantile(Jira_bugs$time_spent, .25)
Q3ts <- quantile(Jira_bugs$time_spent, .75)
Q1rt <- quantile(Jira_bugs$repair_time, na.rm = TRUE, .25)
Q3rt <- quantile(Jira_bugs$repair_time, na.rm = TRUE, .75)
```
```{r no_outliers, echo=FALSE}
for(i in 1:length(Jira_bugs$Priority)){
  if((Jira_bugs$time_spent[i] < Q1ts) || (Jira_bugs$time_spent[i] > Q3ts)){
    Jira_bugs$time_spent_no_outliers[i] <- NA
  }
  else{
    Jira_bugs$time_spent_no_outliers[i] <- Jira_bugs$time_spent[i]
  }
  if(is.na(Jira_bugs$repair_time[i]) == FALSE){
    if(((Jira_bugs$repair_time[i] < Q1rt) || (Jira_bugs$repair_time[i] > Q3rt))){
      Jira_bugs$repair_time_no_outliers[i] <- NA
    }
    else{
      Jira_bugs$repair_time_no_outliers[i] <- Jira_bugs$repair_time[i]
    }
  }
}
```
...
write_csv(Jira_bugs, "NSWCCD_data.csv")
