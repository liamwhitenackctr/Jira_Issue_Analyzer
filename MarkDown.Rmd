...
Jira_reduced <- Jira_bigger %>%
    select(priority, time_spent, repair_time, date_created, date_updated, date_resolved, unix_created, unix_updated, unix_resolved, parent, type, Assignee, Reporter, Completed, Status, Summary)
```
...
## Summary Statistics
```{r}
Number_of_weeks <- (max(Jira_reduced$unix_updated) - min(Jira_reduced$unix_created)) * (1 / (60 * 60 * 24 * 7))
```
```{r}
Jira_reduced %>%
  group_by(priority) %>%
