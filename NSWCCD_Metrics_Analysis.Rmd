...
Jira_tidy <- read_csv("Informedb_NSWCCD_data.csv")
```
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      selectInput(
        inputId = "xvar",
        label = "X Variable:",
        choices=c("Date Created",  "Date Updated",  "Date Resolved", "Characters (Without Outliers)"),
        selected = "Date Created"
      ),
...
server <- function(input, output) {
  ...
  group_by_(input$group) %>%
    summarise( 
      ...
      `Characters in Description` = sum(Characters)/n()
    )
  })
}
